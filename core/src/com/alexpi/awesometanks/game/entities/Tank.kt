...
fun World.createPlayer(assetManager: AssetManager, physicsWorld: PhysicsWorld, position: Vector2, gameRepository: GameRepository): Int {
    ...
    edit(tankId).add(LinearMovementComponent(speed = 150f + gameRepository.getUpgradeLevel(PerformanceUpgrade.SPEED) * 10f))
    ...
    val rotationSpeed = 4.2f + gameRepository.getUpgradeLevel(PerformanceUpgrade.ROTATION) * 1.5f
    val weaponId = createWeapon(assetManager, bodyComponent, rotationSpeed)

    edit(tankId).add(WeaponComponent(weaponId))
    return tankId
}

fun World.createWeapon(assetManager: AssetManager, bodyComponent: BodyComponent, rotationSpeed: Float): Int {
    val weaponId = create()
    println("tank weapon id = $weaponId")
    edit(weaponId).add(SmoothRotationComponent(rotationSpeed, 1/40f))
